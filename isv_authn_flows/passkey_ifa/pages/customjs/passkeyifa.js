...
function commonOnLoad(scriptElement) {
    // look for themeId if present
    if (scriptElement != null && scriptElement.textContent != null) {
        let tagJson = JSON.parse(scriptElement.textContent);
        passkeyifaThemeId = tagJson["themeId"];
        targetURL = tagJson["targetURL"];
    }

    console.log("passkeyifaThemeId: " + passkeyifaThemeId);
}
