...
function mediate_attestation_result() {
    ...
    // DEMO - Added for FIDO2App
    let auditRecordObj = {
        timestamp: (new Date()).toISOString(),
        rawRequestData: context.rawRequestData,
        username: context.requestData.options.user.name,
        displayName: context.requestData.options.user.displayName,
        credentialId: context.requestData.registration.credentialId,
        publicKey: context.requestData.registration.credentialPublicKey
      };
      IDMappingExtUtils.logAuditEvent​(context.requestData.registration.username, JSON.stringify(auditRecordObj), true);    
}
...
function mediate_assertion_result() {
    ...
    // first stuff that the FIDO2App expects in a response
    credentialData.put("fidoLoginDetails", JSON.stringify(context));
    credentialData.put("displayName", context.requestData.registration.username);
    credentialData.put("email", context.requestData.registration.username);
    credentialData.put("AUTHENTICATOR_FRIENDLY_NAME", context.requestData.registration.nickname);
    if (context.requestData.registration["metadata"] != null) {
        if (context.requestData.registration.metadata["icon"] != null)  {
            credentialData.put("AUTHENTICATOR_ICON", context.requestData.registration.metadata.icon);
        }
    }
  
    // now log a custom audit record
    if (context.requestData.authData.extensions != null && context.requestData.authData.extensions["txAuthSimple"] != null) {
        let timestamp = (new Date()).toISOString();
        IDMappingExtUtils.traceString("Processing transaction. Timestamp: " + timestamp +
            " CredentialId: " + context.requestData.registration.credentialId +
            " Txn: " + context.requestData.authData.extensions.txAuthSimple);

            // create a custom audit log entry too
        let auditRecordObj = {
          timestamp: timestamp,
          rawRequestData: context.rawRequestData,
          signature: context.requestData.signature,
          txn: context.requestData.authData.extensions.txAuthSimple,
          username: context.requestData.registration.username,
          displayName: context.requestData.registration.username,
          credentialId: context.requestData.registration.credentialId,
          publicKey: context.requestData.registration.publicKey
        };
        IDMappingExtUtils.logAuditEvent​(context.requestData.registration.username, JSON.stringify(auditRecordObj), true);
    }
